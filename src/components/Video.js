...
const Video = (props) => {
  ...
  return (
    data.length > 0 ?
      <>
        <span className='align-middle'>{data.filter((item) => item.type === "Trailer").map((dataTrailer, index) => (index < 1 && dataTrailer ? <a className='ms-3 aLink' key={index} href={"https://www.youtube.com/watch?v=" + dataTrailer.key} target="_blank" rel="noreferrer"><IoPlay className='me-2 mb-1' />Play trailer</a> : null))}</span>
