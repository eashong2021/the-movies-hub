      ...
      {data.length !== 0 ? (
        <>
          <LazyLoadImage
            ...
            src={POSTER_PATH + WIDTH_ORIGINAL + data.backdrop_path}
            effect="opacity"
            alt={data.title}
            onError={(e) => {
          ...
          <div className="container">
            <div className="row">
              <div className="col-12 col-md-2 text-dark">
                <LazyLoadImage
                  className="img-poster rounded"
                  src={POSTER_PATH + WIDTH_500 + data.poster_path}
                  effect="opacity"
                  alt={data.title}
                  onError={(e) => {
                    e.target.onerror = null;
                ...
                {data.homepage ? (
                  <div className="hide-sm ">
                    <a
                      ...
                      className="text-dark"
                    >
                      Homepage <FiExternalLink className="ms-2 mb-1" />
                    </a>
                  </div>
                ) : null}
                {/* <div className="bebasFont hide-md mt-3 mb-3 text-break ">
                  {data.title}
                </div> */}
                {data.homepage ? (
                  <div className="hide-md mt-3">
                ...
                <div className="hide-sm">
                  {/* <div>
                    <ExternalId id={id} name="movie" />
                  </div> */}
                  <div>
                    {data.status ? (
                      <>
                        <div className="text-bold pt-3">Status</div>
                        <div className="text-secondary">{data.status}</div>
                      </>
                    ) : null}
                  </div>
                  <div>
                    {data.original_language ? (
                      <>
                        <div className="mt-4 text-bold">Language</div>
                  ...
                  <div>
                    {data["revenue"]?.length > 0 ? (
                      <>
                        <div className="mt-4 text-bold ">Revenue</div>
                        <div className="text-secondary">
                  ...
                  <div>
                    {data["budget"]?.length > 0 ? (
                      <>
                        <div className="mt-4 text-bold text-dark">Budget</div>
                        <div className="text-secondary">
              ...
              <div className="col-12 col-md-10 mt-3">
                ...
                <div>
                  {data.release_dates
                    ? data.release_dates["results"].map((dataCerti, index) =>
                        dataCerti.iso_3166_1 === "US" ? (
                          <div
                            className="text-secondary badge py-2"
                            key={index}
                          >
                            {dataCerti["release_dates"].map(
                              (dataC, index, row) =>
                                index + 1 === row.length
                                  ? dataC.certification
                                    ? dataC.certification
                ...
                <div className="text-secondary">
                  {data["genres"]
                    ? data["genres"].map((dataGenre, index) => (
                ...
                <div>
                  {data.vote_average ? avg(data.vote_average) : null}{" "}
                  <span
                    className="stars"
                    style={{
            ...
            <div className="hide-md mt-5">
              ...
              <div>
                {data["budget"]?.length > 0 ? (
                  <>
                    <div className="mt-4 text-bold">Budget</div>
                    <div className="text-secondary">
                      {currency(data.budget)}
                    </div>
                  </>
        ...
        <div className="vh-100 d-flex justify-content-center align-items-center text-center">
          <div>
            ...
            <div className="aLink" onClick={history.goBack}>
              &#8592; Go Back
            </div>
          </div>
        </div>
